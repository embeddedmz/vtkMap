...
vtkStandardNewMacro(vtkBingLayer)

  extern unsigned int tileNotAvailable_png_len;
extern unsigned char tileNotAvailable_png[];

//----------------------------------------------------------------------------
vtkBingLayer::vtkBingLayer()
  : vtkFeatureLayer()
{
  this->BaseOn();
  this->MapTileServer = strdup("t0.tiles.virtualearth.net");
  this->MapTileExtension = strdup("jpg");
  this->MapTileAttribution = strdup("(c) Microsoft Bing");
...
//----------------------------------------------------------------------------
void vtkBingLayer::PrintSelf(ostream& os, vtkIndent indent)
{
  this->Superclass::PrintSelf(os, indent);
}

//----------------------------------------------------------------------------
void vtkBingLayer::SetMapTileServer(const char* server,
...
void vtkBingLayer::Update()
{
  ...
  if (!vtkBingLayer::VerifyImageFile(nullptr, TileNotAvailableImagePath))
  {
...
//----------------------------------------------------------------------------
void vtkBingLayer::SetCacheSubDirectory(const char* relativePath)
{
  if (!this->Map)
  {
...
//----------------------------------------------------------------------------
bool vtkBingLayer::DownloadImageFile(std::string url, std::string filename)
{
...
//----------------------------------------------------------------------------
bool vtkBingLayer::VerifyImageFile(FILE* fp, std::string filename)
{
...
void vtkBingLayer::SelectTiles(std::vector<vtkSmartPointer<vtkMapTile> >& tiles,
  std::vector<vtkMapTileSpecInternal>& tileSpecs)
{
...
void vtkBingLayer::InitializeTiles(
  std::vector<vtkSmartPointer<vtkMapTile> >& tiles,
  std::vector<vtkMapTileSpecInternal>& tileSpecs)
{
  ...
  for (; tileSpecIter != tileSpecs.end(); tileSpecIter++)
  {
    ...
    else
    {
      ...
      // execution of a program using vtkMap and vtkBingLayer.
      // Update tile cache :
      this->AddTileToCache(
        spec.ZoomXY[0], spec.ZoomXY[1], spec.ZoomXY[2], tile);
    }
...
void vtkBingLayer::RenderTiles(std::vector<vtkSmartPointer<vtkMapTile> >& tiles)
{
  if (tiles.size() > 0)
  {
    // Remove old tiles
...
//----------------------------------------------------------------------------
void vtkBingLayer::AddTileToCache(int zoom, int x, int y, vtkMapTile* tile)
{
...
//----------------------------------------------------------------------------
vtkSmartPointer<vtkMapTile> vtkBingLayer::GetCachedTile(int zoom, int x, int y)
{
...
//----------------------------------------------------------------------------
void vtkBingLayer::MakeFileSystemPath(vtkMapTileSpecInternal& tileSpec,
  std::stringstream& ss)
{
  ss.str("");

  const std::string quadKey = TileXYToQuadKey(
    tileSpec.ZoomRowCol[1], tileSpec.ZoomRowCol[2], tileSpec.ZoomRowCol[0]);

  ss << this->GetCacheDirectory() << "/" << quadKey << "."
     << this->MapTileExtension;
}

//----------------------------------------------------------------------------
void vtkBingLayer::MakeUrl(vtkMapTileSpecInternal& tileSpec,
  std::stringstream& ss)
{
  ss.str("");
  ...
  ss << "http://" << this->MapTileServer << "/tiles/a" << quadKey
     << ".jpeg?g=854&token=A";
}

//----------------------------------------------------------------------------
std::string vtkBingLayer::TileXYToQuadKey(int tileX,
  int tileY,
  int levelOfDetail)
{
